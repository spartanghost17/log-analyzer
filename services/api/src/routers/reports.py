...
async def get_reports(
        ...
        date_from: Optional[str] = Query(None, description="Filter reports from this date (YYYY-MM-DD)"),
        date_to: Optional[str] = Query(None, description="Filter reports up to this date (YYYY-MM-DD)"),
    ...
    - **date_from**: Filter reports from this date (YYYY-MM-DD format)
    - **date_to**: Filter reports up to this date (YYYY-MM-DD format)

    Returns analysis reports generated by the LLM analyzer service.
    """

    # Try cache
    cache_key = cache.cache_key("reports", limit=limit, offset=offset, date_from=date_from, date_to=date_to)
    cached = await cache.get(cache_key)
    if cached:
        return cached

    # Query database
    reports = await db.get_analysis_reports(
        limit=limit, 
        offset=offset,
        date_from=date_from,
        date_to=date_to
